...
The program demonstrates different PTX techniques including inline assembly, device function pointers, and cooperative groups. Each device-side method clears the output buffer first and validates all 1,000 results, so an incomplete launch cannot be hidden by values left by an earlier method.
